 ...
 */
const getProductDataListFromProductUuid = (
    product_uuid_list,
    transaction
) => {
    return new Promise(async (resolve, reject) => {
        try {
            //
            const placeholder = "?,".repeat(product_uuid_list.length).slice(0, -1);

            const response = await query(
                "SELECT * FROM `PRODUCT_MASTER` WHERE `product_uuid` IN(" + placeholder + ");",
                product_uuid_list,
                transaction
            ).catch((err) => {
                throw err;
...
export default getProductDataListFromProductUuid;